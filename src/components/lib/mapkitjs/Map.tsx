...
const CreateMap: React.FC<{
  ...
  return (
    <MapProvider context={{ mapkit, map }}>
      <MapContainer mapRef={mapRef}>{children}</MapContainer>
    </MapProvider>
  );
};
...
> = ({ tokenOrCallback, language = 'en', mapkit, map, mapRef, ...props }) => {
  ...
  if (mapRef) {
    ...
    const mapBox = <CreateMap mapOptions={props} />;
    ...
    return (
      <MapkitProvider tokenOrCallback={tokenOrCallback} language={language}>
        {mapBox}
      </MapkitProvider>
    );
  }
