...
export const CustomImageAnnotationV1: React.FC<CustomImageAnnotationProps> = ({
  worker,
  offset,
  selectedSize,
  deselectedSize,
  ...
  useEffect(() => {
    let annotationObject = annotationRef.current;

    annotationObject = createImageAnnotation(mapkit, worker, false, true, []);
    if (!annotationObject) return;
