...
import { isTreeScrollable } from './styles/idScrollable';
import './styles/main.css';
import { DIRECTION } from './styles/types';
...
const PullToRefresh: React.FC<PullToRefreshProps> = ({
  ...
  const onTouchMove = (e: MouseEvent | TouchEvent): void => {
    ...
    containerRef.current && containerRef.current.classList.add('ptr--dragging');

    if (
      currentY < startY ||
      (currentY - startY) / Math.abs(currentX - startX) <= 7 / 6
    ) {
      isDragging = false;
      return;
    }
